...
| -- | ---- | -------- | ---- | ------ |
| TD-17 | AI-usage cost: bulk/low-stakes jobs (doc drafts, summaries, daily-digest first pass) hit the Claude API at Opus rates instead of the local LiteLLM+Ollama tier we already run on the t630 (stage 10). Point cheap work local, reserve Opus for design/decision/review. **Blocked on TD-14** — must fix the privacy fail-open before routing more to local. See [`process-efficiency.md`](process-efficiency.md) §2. | P2 | localDNS/DESIGN | Open (found 2026-06-16) |
| TD-16 | Session-protocol overhead: `CLAUDE.md` tells the agent to read 4 CTO + 6 CFO files at every session start (10 files before any task); most tasks need ~2. Make the protocol conditional (read portfolio always; the rest on demand). See [`process-efficiency.md`](process-efficiency.md) §4. | P3 | DESIGN | Open (found 2026-06-16) |
| TD-15 | Standing context tax: ~14,600 tokens of `CLAUDE.md` load into every session (worst: localDNS ~5.1k, DESIGN ~4.5k — full deploy/config/funnel tables that aren't needed per-session) + the house-style block is duplicated verbatim across all 7 repos. Split each `CLAUDE.md` into a thin briefing + linked deep reference; centralize house-style. Target ~14.6k→~4–5k. See [`process-efficiency.md`](process-efficiency.md) §1. | P2 | all repos | Open (found 2026-06-16) |
