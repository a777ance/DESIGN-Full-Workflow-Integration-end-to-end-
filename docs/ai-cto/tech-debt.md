...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI process efficiency: standing context (large `CLAUDE.md` + full NARF/ZORT session-start reads) re-ingested every run; Opus-`high` used for mechanical routines that Haiku/Sonnet/`low` or `check-docs.py` would handle; full review runs daily where weekly + event-triggered would do. Free wins (config/cadence/prompting), not spend. Full analysis + 6 next steps in `docs/ai-cto/2026-06-14-ai-process-efficiency-audit.md`. | P2 | All | Open (found 2026-06-14) |
