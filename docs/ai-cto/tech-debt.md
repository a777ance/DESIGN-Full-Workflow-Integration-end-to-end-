...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI process cost: prompt caching not enabled on the LiteLLM→Claude path; `cloud-overflow`/`cloud-explore`/`cloud-vision` all pinned to Opus 4.8 (priciest tier) instead of Haiku/Sonnet first; non-interactive jobs (monthly statements, log summaries) not on the Batch API; shared house-style block duplicated across 7 CLAUDE.md (~1,040 lines load when a session opens at the parent dir). Full analysis + ranked fixes in `docs/ai-cto/ai-process-efficiency.md`. Note: the Opus-overflow fix is the same `config.yaml` edit as TD-14's privacy fallback. | P2 | localDNS / DESIGN | Open (found 2026-06-27) |
