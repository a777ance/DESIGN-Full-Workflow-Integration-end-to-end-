...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI process token waste: every session front-loads ~10 files (NARF's 4 + ZORT's 6 "read at session start") plus ~10.7K tokens of always-loaded CLAUDE.md across 6 repos — most irrelevant to the task. House-style block + repo table + money-flow diagram are duplicated verbatim across all 6 CLAUDE.md files. Fix: make session-start reads conditional, de-dup boilerplate into one linked file, trim localDNS (326 lines) + DESIGN (295) to the ~200-line budget. Full audit + ranked fixes in `docs/ai-cto/process-efficiency.md`. | P2 | all | Open (found 2026-06-18) |
