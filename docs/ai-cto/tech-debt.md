...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI-process token bloat: every `CLAUDE.md` exceeds Anthropic's 200-line guidance (localDNS 326, DESIGN 295, MARKETING 214) and is loaded in full at every session start; scheduled routines load *all* repos' CLAUDE.md. The ~18-line house-style block is duplicated verbatim across all 7 repos. Fix: trim each CLAUDE.md <200 lines, move deploy/verify/checklist blocks to on-demand Skills, de-dup house style to one canonical copy, scope routines to fewest repos. Full review + prompt/routing/caching recommendations in `docs/ai-cto/process-efficiency-2026-06.md`. | P2 | all | Open (found 2026-06-20) |
