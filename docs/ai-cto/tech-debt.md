...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI process token waste: (a) prompt caching is unused on the stable CLAUDE.md/tool-def prefix (60–90% input-token win, break-even after one read); (b) the ~290-word house-style block + the three-repo/roles tables are pasted verbatim across all 7 CLAUDE.md files (paid 7× per multi-repo load, drift risk — Azure-lab's copy already differs); (c) frontier model (Opus) used on routine doc work Sonnet/Haiku would handle; (d) routines/sessions load all 7 briefings when one repo is in scope. Fix order: enable caching → extract `HOUSE-STYLE.md` + slim each CLAUDE.md to one screen → set model defaults by task → scope routines. The "push more to local" half is gated on TD-14 (fail-closed). Full analysis: `docs/ai-cto/reviews/2026-06-25-process-token-efficiency-review.md`. | P2 | DESIGN (all repos) | Open (found 2026-06-25) |
