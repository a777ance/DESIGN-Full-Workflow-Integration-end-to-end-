...
| -- | ---- | -------- | ---- | ------ |
| TD-17 | House-style block (~350 words / ~1.5K tokens) is duplicated verbatim in all 7 CLAUDE.md files — every multi-repo session carries it multiple times, and a rule change means editing 7 files + busting the prompt cache everywhere. Fix: one canonical `docs/house-style.md` (or a shared skill), each CLAUDE.md links to it. | P2 | all | Open (found 2026-06-14, process-efficiency review) |
| TD-16 | CLAUDE.md baseline too heavy: ~14.6K tokens load every turn when a session is scoped to all repos (localDNS ~5.1K + DESIGN ~4.5K + MARKETING ~2.7K + …). Fix: scope sessions/routines to one repo; trim each CLAUDE.md to a lean <2K-token router and move always-loaded reference tables (deploy paths, nftables runbook) to on-demand docs. | P2 | all | Open (found 2026-06-14) |
| TD-15 | NARF/ZORT daily reviews re-derive the same conclusion at full Opus cost. 11 daily reviews (06-04→06-14) mostly conclude "nothing shipped since 06-07, blocked on t630." Fix: diff-gate the routine (cheap Haiku check of CHANGELOG/`git log` first; full Opus pass + write only when something changed; stay silent otherwise). Candidate for native "Dreaming" memory-curation. | P2 | DESIGN | Open (found 2026-06-14) |
