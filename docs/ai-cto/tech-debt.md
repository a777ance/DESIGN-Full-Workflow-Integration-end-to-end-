...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI process token waste: the 38-line "House style" block is duplicated verbatim across all 6 CLAUDE.md files (~6.6 KB pure dup), and a multi-repo session loads ~14.6K tokens of CLAUDE.md before any work. Also: bulk/mechanical work runs on Opus instead of the local LiteLLM router (stage 10) we already built. Fix: factor house-style into one `docs/house-style.md`, trim each CLAUDE.md to every-session essentials, move situational guidance into Skills, and route cheap tasks local (after TD-14 is fixed so sensitive data fails closed). Full analysis + sources: `docs/ai-cto/ai-process-efficiency.md`. | P2 | all | Open (found 2026-06-14) |
