...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | Claude-Code setup tax: ~14.6k tokens of `CLAUDE.md` load across 6 repos every session (58.4 KB; house-style block duplicated verbatim ×6 ≈ 7 KB dead weight), plus §5/§6 force ~11 unconditional session-start file reads. Most is irrelevant to any given task and re-paid on every cold/scheduled start (cache TTL is now 5 min). Fix: trim each `CLAUDE.md` to rules-only (<1,500 tok), de-dupe house-style to one linked file, move reference tables to README, make §5/§6 reads conditional. Full analysis + hybrid-router and model-tier recommendations in `docs/ai-cto/process-efficiency-2026-07-02.md`. | P2 | all | Open (found 2026-07-02) |
