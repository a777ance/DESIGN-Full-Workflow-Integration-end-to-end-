...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | CLAUDE.md files are README mirrors loaded into context every session/turn (~10.7k tokens across all repos; ~3.6k in a `localDNS` session, ~3.5k in DESIGN). The deploy-paths table, known-issues table, nftables checklist and verification blocks belong in README and should be read on demand, not paid for every turn. Also: the ~250-word house-style block is byte-identical in all 7 repos. Fix: trim each CLAUDE.md to a <800-word pointer file; collapse house-style to a 4-line stub + link to one canonical `docs/house-style.md`. Compounds with prompt caching. See `docs/ai-cto/ai-process-efficiency.md` §A. | P2 | all | Open (found 2026-06-14) |
