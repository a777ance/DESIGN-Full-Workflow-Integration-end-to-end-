...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | User↔AI process token tax: opening Claude Code at `/home/user` injects all six repos' `CLAUDE.md` (~1,040 lines ≈ ~12K tokens) on every turn, and a scheduled run on 2026-06-28 injected `localDNS/CLAUDE.md` twice (~660 dup lines). Each `CLAUDE.md` runs 50–326 lines (target ~200), and the house-style block is copy-pasted verbatim across all six. Fix: one-repo-per-session habit, trim each `CLAUDE.md` toward ~200 lines (push deep tables into linked READMEs), de-dup house-style via a shared import, add `.claudeignore` where build dirs exist. Full review + checklist: `docs/ai-cto/process-efficiency-review.md`. | P2 | all | Open (found 2026-06-28) |
