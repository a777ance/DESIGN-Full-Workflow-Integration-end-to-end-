...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI process cost: CLAUDE.md files are large (~400 lines in localDNS) and the ~40-line house-style block is duplicated byte-for-byte across all 7 repos — all loaded into context every session and re-sent every turn, now metered at full API rates since the 2026-06-15 Claude Code pricing change. Fix: trim each CLAUDE.md to a briefing + on-demand links; extract one shared `STYLE.md`. Also: enable `ENABLE_PROMPT_CACHING_1H`, pin cheap routines to Haiku/Sonnet (not Opus), route log/diff triage local-first via the LiteLLM router (after TD-14 fail-closed fix). Full analysis: `docs/ai-cto/reviews/2026-06-16-process-efficiency-review.md`. | P2 | all | Open (found 2026-06-16) |
