...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | Process/token efficiency: CLAUDE.md files total ~14.6K resident tokens/turn (localDNS 20.5KB, DESIGN 18KB); the "House style" block is duplicated verbatim in all 7 repos; session-start reading ritual (NARF 4 files, ZORT 6) is unconditional. Fix: trim CLAUDE.md under ~200 lines, move deep tables/checklists to on-demand Skills, de-duplicate house style to one canonical file, gate the CTO/CFO reads on actually doing that work. Also: route more simple work to the local LLM (after TD-14), batch scheduled LLM jobs, right-size model/effort, capture a `/usage` baseline. Full audit: [`process-efficiency-2026-06.md`](process-efficiency-2026-06.md) | P2 | ALL | Open (found 2026-06-14) |
