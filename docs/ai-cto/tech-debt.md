...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | CLAUDE.md context tax: ~14.6K tokens of CLAUDE.md load on *every* turn (all in-scope repos at once), and the ~250-word house-style block is duplicated verbatim across all 7 files — paid before any work happens. Fix: trim each CLAUDE.md to always-true facts (demote stage/walkthrough detail to READMEs read on demand), de-duplicate house style into one canonical file the others link to, scope sessions to one repo where possible. Benchmark: 3,847→312 tok trim = 91.9% context cut, no quality loss. See `docs/ai-cto/reviews/2026-06-28-process-efficiency-review.md`. | P2 | ALL | Open (found 2026-06-28) |
