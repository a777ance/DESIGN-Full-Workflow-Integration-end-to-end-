...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI-process token bloat: sessions opened at `/home/user` load all six `CLAUDE.md` (~14.6k tokens, never evictable) regardless of the repo in play; the house-style block is duplicated verbatim across all six; localDNS/DESIGN `CLAUDE.md` carry rationale that already lives in their linked context files. Fix: per-repo cwd sessions, house style → one `~/.claude/CLAUDE.md`, trim `CLAUDE.md` to lookup-table size. Also: route repo chores (lint, commit msgs, summaries) through the local LLM router, and move async jobs (monthly statements, doc-lint) to the Batch API (50% off). Full audit: `docs/ai-cto/reviews/2026-07-02-ai-process-efficiency.md`; turnkey steps: `docs/ai-cto/token-efficiency-rollout.md` + canonical `HOUSE-STYLE.md`. | P2 | ALL | Open (found 2026-07-02) |
