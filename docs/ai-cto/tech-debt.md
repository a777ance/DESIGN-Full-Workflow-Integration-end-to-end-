...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI-loop cost/token efficiency: day-to-day work runs on Opus 4.8 (1M) with full multi-repo `CLAUDE.md` re-loaded each turn and no prompt caching — the most expensive way to do mostly doc/Markdown/JSON edits. Levers: right-size routine work to Sonnet 4.6/Haiku 4.5 (and the local box we own); enable prompt caching on the repo briefing (cached reads = 0.1×); batch the non-interactive runs (50% off); point `cloud-overflow` failover at Haiku not Opus; scope prompts with an output contract. Full analysis + sources: `docs/ai-cto/process-efficiency-2026-06-16.md`. Credible 5–10× cut on the recurring review/edit slice. | P2 | all | Open (found 2026-06-16) |
