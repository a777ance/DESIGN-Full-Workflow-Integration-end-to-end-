...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | AI process inefficiency: `CLAUDE.md` files are 5–10× the ~500-token best-practice size and reload every turn (localDNS ~5.1K tok, DESIGN ~4.5K, MARKETING ~2.7K); House-style block is duplicated verbatim across all 7 repos; session-start rituals auto-read ~10 files. Day-to-day work also bypasses the LiteLLM ladder we already run, so the easy 60–70% of tasks hit Claude instead of the local model. Fix: slim the big-three CLAUDE.md to a thin core, de-dupe House style to one linked file, make session-start reads on-demand/Skills, route mechanical tasks (doc-check, roster validation, data prep) local (gated on TD-14), and add prompt-caching+Batch to statement generation if it calls a hosted model. Full audit + sources: `docs/ai-cto/ai-process-efficiency.md`. | P2 | DESIGN (cross-repo) | Open (found 2026-06-14) |
