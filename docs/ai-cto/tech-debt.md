...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | Per-session context bloat: ~1,040 lines of CLAUDE.md load every session, plus NARF (4-file) + ZORT (6-file) session-start reading rituals — a "quick question" spends 20–30K tokens hydrating before any work. Fix: prune CLAUDE.md to rules+map (link out the reference tables), convert the session-start reads to on-demand Skills, keep prefixes byte-stable for prompt caching. Full plan + routing/batch/caching wins in `docs/ai-cto/process-efficiency.md`. | P2 | all repos | Open (found 2026-06-16) |
