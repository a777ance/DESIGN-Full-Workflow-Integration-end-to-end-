...
| -- | ---- | -------- | ---- | ------ |
| TD-15 | Dev-loop token waste: ~24k tokens (7 `CLAUDE.md` + 9 NARF/ZORT mandatory reads, incl. a 3,681-word `metrics.md`) load at every session start and re-bill on every cache expiry; the house-style block is duplicated verbatim in 6 `CLAUDE.md` files; the dev process never uses the local Odin/LiteLLM tier or cheaper models. Fix set + measurements in [`process-efficiency.md`](process-efficiency.md). Top levers: lazy session-start reads (~60% startup cut), trim/de-dupe `CLAUDE.md`, model-match (Sonnet/Haiku/local for chores), batch API for non-interactive jobs, lint the house style. | P2 | DESIGN | In progress (2026-07-01): structural fixes shipped across all repos — NARF/ZORT session-start reads made lazy (only `portfolio.md` mandatory); house-style block de-duped to canonical `docs/house-style.md` + a 4-line summary in all 6 `CLAUDE.md` (chronikomicon has none); `metrics.md` split (3,681→~500 words, log → `metrics-history.md`) with a delta-only logging rule. Each on its `claude/*` branch. Remaining: model-match + batch API are usage habits (no code to land); the house-style linter (§⑦) is not yet built. |
